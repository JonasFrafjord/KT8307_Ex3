...
#Cu010 = open('Cu010More.dat')                      #Opens the file
#Cu110 = open('Cu110More.dat')                      #Opens the file
#Fq = open('FQ.dat')
...
#    exit()
    ...
    plt.plot(depth[SILoad:EILoad], [pressureLoad(i, alphaL, n) for i in depth[SILoad:EILoad]], label=r'$F = {:.2f}*h^{{{:.2f}}}$'.format(alphaL,n))
#    plt.show(),exit()
    
    plt.xlabel(r'Displacement $h$ [nm]')
    plt.ylabel(r'Load $F$ [$\mu$ N]')
    plt.title(r'Nano indentation on Copper in [111]', fontsize = 26, y=1.04)
    plt.ticklabel_format(style='sci', axis='y', scilimits=(0,0))
    plt.legend(loc='best')
    plt.show(),exit()
...
def main():
#    routine1(Cu)
    routine1(Cu111, [0.01, 0.7], [0.0, 0.25])
#    routine1(Cu010, [0.01, 0.9], [0.05, 0.775])
#    routine1(Cu110, [0.01, 0.8], [0.03, 0.5])
    return 0
